...
RasterBandPixelBlockWalker::~RasterBandPixelBlockWalker(void)
{
...
RasterBandPixelBlockWalker::RasterBandPixelBlockWalker( GDALDataset* raster,int band,int width,int height,GDALDataset* aoiRaster /*= nullptr*/, int aoiBand/*=0*/,int mlimit /*= 16*/ )
{
	new (this)RasterBandPixelBlockWalker(raster,raster->GetRasterBand(band),width,height,aoiRaster,aoiBand,mlimit);
}

RasterBandPixelBlockWalker::RasterBandPixelBlockWalker( GDALDataset* raster,GDALRasterBand* band,int width,int height,GDALDataset* aoiRaster /*= nullptr*/, int aoiBand/*=0*/,int mlimit /*= 16*/ )
{
...
RasterBandPixelBlockWalker::RasterBandPixelBlockWalker( GDALDataset* raster,int band,int width,int height,int xOffset /*= 0*/, int yOffset /*= 0*/, int xSize /*= -1*/, int ySize /*= -1*/,int mlimit /*= 16*/ )
{
	new (this)RasterBandPixelBlockWalker(raster,raster->GetRasterBand(band),width,height,xOffset,yOffset, xSize,ySize,mlimit);
}

RasterBandPixelBlockWalker::RasterBandPixelBlockWalker( GDALDataset* raster,GDALRasterBand* band,int width,int height,int xOffset /*= 0*/, int yOffset /*= 0*/, int xSize /*= -1*/, int ySize /*= -1*/,int mlimit /*= 16*/ )
{
	int xRasterSize = raster->GetRasterXSize();
	int yRasterSize = raster->GetRasterYSize();
...
void RasterBandPixelBlockWalker::ResetReading()
{
...
void RasterBandPixelBlockWalker::ForEachBlock(FOREACHBLOCK_FUNC proc, void* user)
{
	...
	do 
	{
		for (int y=yReadBegin;y<yReadEnd;y++)
		{
			for (int x=xReadBegin; x<xReadEnd; x++)
			{
				...
				{
					int xAoi = (int)_tgis_round((double)(x-_xOffset)*_xAoiSize/_xSize + (double)(_xReadSize+1)*_xAoiSize/(2*_xSize),0);
					int yAoi = (int)_tgis_round((double)(y-_yOffset)*_yAoiSize/_ySize + (double)(_yReadSize+1)*_yAoiSize/(2*_ySize),0);
